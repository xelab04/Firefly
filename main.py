...
async def verify(ctx, phone_num):
    ...
    if False in [num in "0123456789" for num in phone_num] or already_reg(user, phone_num, rows):
        ...
        if already_reg(user, phone_num, rows):
            print(f"{phone_num} {user} already registered")
        if False in [num in "0123456789"  for num in phone_num]:
            print("Phone num has invalid characters")
...
def already_reg(discord_username, phone_no, rows):
    for row in rows:
        if discord_username == row[-1]:
            return True

        if row [-1] != "Paid" and phone_no in row:
